...
class AppWindow:
    ...
    def display_shape(self):
        ...
        vertices_uvs = np.copy(vertices[:, :2])
        vertices_uvs += abs(np.min(vertices_uvs))
        vertices_uvs /= np.max(vertices_uvs)

        triangles_uvs = np.array([[1, 1]])
        for t in np.asarray(mesh.triangles):
            for v in t:
                triangles_uvs = np.append(triangles_uvs, [vertices_uvs[v]], axis=0)

        triangles_uvs = triangles_uvs[1:]

        mesh.triangle_uvs = o3d.utility.Vector2dVector(triangles_uvs)
